...
int sum_systematics(const char* config, const char* label) {
    ...
    for (std::size_t i = 0; i < nhists; ++i) {
        TH1F* hnominal = (TH1F*)f[0]->Get(histograms[i].c_str())->Clone(
